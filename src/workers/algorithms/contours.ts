...
export function contoursForLabelIsland(labels: Uint16Array, islandIds: Uint32Array, w: number, h: number, labelIndex: number, islandId: number): number[][][] {
  // Similar to contoursForLabel but only for cells matching both labelIndex AND islandId
  const edges = new Map<string, [number,number][]>()
  const addEdge = (x0:number,y0:number,x1:number,y1:number) => {
    const k = vkey(x0,y0)
    const arr = edges.get(k) || []
  ...
  const isLabelIsland = (x:number,y:number) => {
    if (x<0||y<0||x>=w||y>=h) return false
    const idx = y*w + x
    return labels[idx] === labelIndex && islandIds[idx] === islandId
  }
  // generate edges along grid lines with CCW orientation for interior
  for (let y=0;y<h;y++) {
    for (let x=0;x<w;x++) {
      if (!isLabelIsland(x,y)) continue
      // top edge if neighbor above is different
      if (!isLabelIsland(x, y-1)) addEdge(x, y, x+1, y) // (x,y)->(x+1,y)
      // right edge if neighbor to right is different
      if (!isLabelIsland(x+1, y)) addEdge(x+1, y, x+1, y+1) // (x+1,y)->(x+1,y+1)
      // bottom edge if neighbor below is different
      if (!isLabelIsland(x, y+1)) addEdge(x+1, y+1, x, y+1) // (x+1,y+1)->(x,y+1)
      // left edge if neighbor to left is different
      if (!isLabelIsland(x-1, y)) addEdge(x, y+1, x, y) // (x,y+1)->(x,y)
    }
  }
  // trace loops (same as original)
  const used = new Set<string>()
  const loops: number[][][] = []
  for (const [startKey, outs] of edges.entries()) {
    for (const out of outs) {
...
export function contoursSvgForAllLabels(labels: Uint16Array, w: number, h: number, cellSize: number, fillByIndex: (i:number)=>string, backgroundIndex: number, widthPx: number, heightPx: number, islandIds?: Uint32Array, islandsByLabel?: Map<number, number[]>) {
  const parts: string[] = []
  ...
  if (islandIds && islandsByLabel) {
    // Generate separate path for each island
    for (const idx of present) {
      if (idx === backgroundIndex) continue
      const islandIdsForLabel = islandsByLabel.get(idx) || []
      for (const islandId of islandIdsForLabel) {
        const loops = contoursForLabelIsland(labels, islandIds, w, h, idx, islandId)
        if (!loops.length) continue
        const d = contoursToPathData(loops, cellSize)
        const fill = fillByIndex(idx)
        parts.push(`<path d="${d}" fill="${fill}" fill-rule="evenodd" data-island-id="${islandId}"/>`)
      }
    }
  } else {
    // Original behavior: combine all islands of same label into single path
    for (const idx of present) {
      if (idx === backgroundIndex) continue
